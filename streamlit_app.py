...
st.set_page_config(page_title="Infosphere Phase Transition", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Infosphere Phase Transition</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Collective information dynamics | Mutual information | Fisher information | Criticality detection</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Infosphere")
...
st.sidebar.markdown(f"**Agents:** {config.N_AGENTS} | **Fisher window:** {config.FISHER_WINDOW}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'infosphere_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Phase order: {etf['phase_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Phase Order"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Phase Order", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Infosphere Phase Order (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Infosphere phase transition** models the ensemble of market participants as an information field.
        - **Mutual information between agents** measures infosphere coherence.
        - **Fisher information** measures sensitivity of the collective state.
        - The phase transition order parameter = geometric mean of coherence and sensitivity.
        - High order → nearing criticality / regime shift.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the phase order parameters.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Infosphere Phase Transition | Collective information dynamics for criticality detection")
