...
from infosphere_phase import infosphere_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = infosphere_score(
            ...
            fisher_window=config.FISHER_WINDOW
        )
        # Add small ticker variation to break ties
    ...
    top_etfs = [{"ticker": t, "phase_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/infosphere_{timestamp}.json"
